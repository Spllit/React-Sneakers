...
function Logo(){
    return (
        <Link to='/'>
            <div className={`${style.logo} ${style.logoContainer}` }>
...
function Home () {
    return(
        <Link to='/'>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M4.225 20.775V9.1L12 3.25l7.775 5.85v11.675h-5.85v-6.8H10.1v6.8ZM6.1 18.9h2.125v-6.8H15.8v6.8h2.1v-8.875L12 5.6l-5.9 4.425Zm5.9-6.65Z" fill='#9B9B9B'/></svg>
        </button>
        </Link>
    )
}
function BagBtn({setBagisOpen}){
    const {total} = React.useContext(AppContext)
    return(
        <button className={style.btn} onClick={setBagisOpen}>
            ...
            <span className={style.cost}>{total}</span>
            <span>руб</span>
        </button>
    )
}
            ...
            <div className={style.container}>
                ...
                <nav>
                    <ul className={style.nav}>
                        <li className={`${style.item} ${style.bag}`}>
                            <BagBtn 
